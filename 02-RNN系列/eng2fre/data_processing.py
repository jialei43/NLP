...
class pairs_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        Returns:
            数据
        """
        index = max(min(index,0),self.sample_num-1)
        ...
        # 添加起始标志
        x_tensor = torch.cat((torch.tensor([SOS_token]), x_tensor), dim=0)
        ...
        # 添加起始标志
        y_tensor = torch.cat((torch.tensor([SOS_token]), y_tensor), dim=0)
...
def data_loader():
    ...
    dataloader = DataLoader(dataset=pairs, batch_size=1, shuffle=True)
    return dataloader
